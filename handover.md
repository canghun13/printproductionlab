...
- Durable QA: added `tools/copy-result-qa.mjs`, which requires exactly 43 calculator pages, no static Copy controls, the versioned shared script and the required common state-management markers. General, navigation, content, encoding, calculation, expanded calculation and layout QA all PASS. HIGH 0; MEDIUM 0; LOW production propagation only until verification after push.
